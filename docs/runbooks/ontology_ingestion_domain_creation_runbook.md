...
Before pages can be processed, they need to be downloaded from http://GOV.UK  and cleaned. 

This tool does two things:

Domain Creation: Groups a list of http://GOV.UK  pages together under a single topic name (a "domain", such as housing or visa-guidance).
...
Follow these steps to download and clean http://GOV.UK  pages for a new domain:

Step 1: Open the Application

Navigate to the deployed application in your web browser:
Link: https://govuk-ai-accelerator-app.integration.publishing.service.gov.uk/ontology/domains
...
Under Provide the name of your domain, type a short, descriptive name using lowercase letters and hyphens (e.g., driving-licences, apprenticeships, tenancy-rules).

Please avoid adding spaces in the names of the domain.
...
Step 3: Paste Your http://GOV.UK  Links

Paste the list of http://GOV.UK  links you want to ingest into the text box.

![URL Entry Screen](images/url_entry_screen.png)

Crucial Formatting Guidelines:
...
If your domain ingestion run fails or skips some pages, check the following:

Missing https:// prefix: Ensure all URLs begin with <https://.> URLs starting directly with www. will be skipped because the system cannot recognise them without the security protocol.

Formatting Characters: Verify there are no commas, colons, or trailing spaces in the URL text box. Even a single trailing comma can cause a URL to fail to load.

Website Domain: The link must belong to a domain ending in .gov.uk (for example, standard pages like <https://www.gov.uk/...> or subdomains like <https://report-error-evisa.homeoffice.gov.uk/...).> Non-government links will be skipped.

Print Links: Avoid pasting print-only pages (e.g., links ending with /print). The tool will strip print options automatically, so paste the standard user-facing web links instead.

Empty Pages: If a page has no text content (for example, if it only contains images or is a landing page with nothing but navigation links), the tool will skip it since there is no text to download.
...
Load Configuration (load_config): Config parameters are merged with default settings derived from the domain name. S3 paths are mapped for output_dir, html_dir, and log_path.
...
Note: Background worker threads execute in-process via ThreadPoolExecutor, so no celery/redis configuration is needed locally.
